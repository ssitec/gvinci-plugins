...
Public Class clsRuleDynamicallyCompiled_R11301
    ...
    Public Shared Function R11301(ByRef con As OracleConnection, ByRef txn As OracleTransaction, ByVal P37254 As Decimal, ByVal P37255 As String) As DataTable
        ' 
        ' 11301 - Copiar Formula Qtde Padrão Insumo - Versão: 0
        ' 
        On Error GoTo Err_R11301
        Dim sCurrComponent As String

        Dim C212716 As Object
        Dim C212716DataType As Short
        Dim QueryC212718 As New Object
        Dim C212718 As Integer
        Dim C212718DataType As Short
        Dim QueryC212720 As New Object
        Dim RsC212720 As New Object
        Dim C212720DataType() As Short
        Dim RsC212720_EOF As Boolean
        Dim C212721 As Boolean
        Dim C212721DataType As Short
        Dim C212722DataType() As Short
        Dim C212724 As Object
        Dim C212724DataType As Short
...
Comp_C212718:
        ...
        ' in_Itens
        sCurrComponent = "in_Itens"
        QueryC212718 = con.CreateCommand()
        QueryC212718.CommandText = QueryC212718.CommandText & " " & "INSERT INTO AKBUSER01.WF_FORMULA_PADR_INUSMO ( WF_FORMULA_PADR_INUSMO.Unid_Medida , WF_FORMULA_PADR_INUSMO.Sigla_Prod , WF_FORMULA_PADR_INUSMO.Sequencia , WF_FORMULA_PADR_INUSMO.Item_Oper , WF_FORMULA_PADR_INUSMO.Codigo_Operador , WF_FORMULA_PADR_INUSMO.Constante , WF_FORMULA_PADR_INUSMO.Total_Fios , WF_FORMULA_PADR_INUSMO.Espec_Item_Operac, WF_FORMULA_PADR_INUSMO.FICHA_TECNICA, WF_FORMULA_PADR_INUSMO.Obrigatorio) "
        QueryC212718.CommandText = QueryC212718.CommandText & " " & "SELECT " &
ConvertValue(P37254, 1, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & " , " &
ConvertValue("TESTE", 3, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & "  , WF_FORMULA_PADR_INUSMO.Sequencia , WF_FORMULA_PADR_INUSMO.Item_Oper , WF_FORMULA_PADR_INUSMO.Codigo_Operador , WF_FORMULA_PADR_INUSMO.Constante , WF_FORMULA_PADR_INUSMO.Total_Fios , WF_FORMULA_PADR_INUSMO.Espec_Item_Operac, WF_FORMULA_PADR_INUSMO.FICHA_TECNICA, WF_FORMULA_PADR_INUSMO.Obrigatorio FROM AKBUSER01.WF_FORMULA_PADR_INUSMO WHERE WF_FORMULA_PADR_INUSMO.Unid_Medida = " &
ConvertValue(P37254, 1, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & " AND  WF_FORMULA_PADR_INUSMO.Sigla_Prod = " &
ConvertValue(P37255, 3, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & " "
        C212718 = QueryC212718.ExecuteNonQuery()
        C212718DataType = AKBTypeConst.cAKBTypeNumeric
        GoTo Comp_C212722

Comp_C212720:

        ' selSiglaExistente
        sCurrComponent = "selSiglaExistente"
        QueryC212720 = con.CreateCommand()
        QueryC212720.CommandText = QueryC212720.CommandText & " " & "SELECT COUNT(WF_SIGLA_PROD_AGRUP.Sigla_Prod) FROM AKBUSER01.WF_SIGLA_PROD_AGRUP WHERE WF_SIGLA_PROD_AGRUP.Sigla_Prod = " &
ConvertValue("TESTE", 3, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & " "
        RsC212720 = QueryC212720.ExecuteReader()
        Dim iC212720 As Short
        ReDim C212720DataType(RsC212720.FieldCount)
        For iC212720 = 0 To RsC212720.FieldCount - 1
            Select Case RsC212720.GetDataTypeName(iC212720).ToUpper()
                Case "NUMBER", "INT32", "DECIMAL", "INT16", "DOUBLE"
                    C212720DataType(iC212720 + 1) = AKBTypeConst.cAKBTypeNumeric
                Case "DATE", "TIMESTAMP"
                    C212720DataType(iC212720 + 1) = AKBTypeConst.cAKBTypeDate
                Case "CHAR", "VARCHAR2", "LONG"
                    C212720DataType(iC212720 + 1) = AKBTypeConst.cAKBTypeString
            End Select
        Next iC212720
        RsC212720_EOF = Not RsC212720.Read()
...
Comp_C212721:
        ...
        ' Sigla Existe?
        sCurrComponent = "Sigla Existe?"
        C212721 = (fn_ConvertValueCompiled(RsC212720.GetValue(0), C212720DataType(1), AKB_DecimalPoint, False) > 0)
        C212721DataType = AKBTypeConst.cAKBTypeLogical
        If C212721 Then
            GoTo Comp_C212718
        Else
            GoTo Comp_C212724
...
Comp_C212722:
        ...
        ' RET1
        sCurrComponent = "RET1"
        Dim tb_C212722 As DataTable = New DataTable()
        tb_C212722.Columns.Add("vRet" & "")
        Dim row_C212722 As DataRow = tb_C212722.NewRow()
        row_C212722(0) = C212716
        tb_C212722.Rows.Add(row_C212722)
        R11301 = tb_C212722
        ReDim C212722DataType(1)
        C212722DataType(1) = C212716DataType
        ReturnDatatype = C212722DataType
...
Comp_C212724:
        ...
        C212724DataType = 4
        C212716 = fn_ConvertValueCompiled("A Sigla " & "TESTE" & " não está cadastrada no sistema.", 3, AKB_DecimalPoint)
        C212724 = True
        GoTo Comp_C212722
...
Err_R11301:
    ...
    '
    Public Shared Function fn_ConvertValueCompiled(ByVal vValue As Object, ByVal lDatatype As Integer, ByVal sDecimalPoint As String,
...
ConvertValue_Error:
    ...
    Public Shared Function ObjTable_NewIdentity(ByRef con As OracleConnection, ByVal TableName As String) As Long
        ...
        ' Retorna a data
        If Rs.Read() Then
            ObjTable_NewIdentity = Rs.GetValue(0)
        End If
        Rs.Close()
    End Function

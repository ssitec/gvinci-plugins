...
Public Class clsRuleDynamicallyCompiled_R9971
    ...
    Public Shared Function R9971(ByRef con As OracleConnection, ByRef txn As OracleTransaction, ByVal P31846 As String) As DataTable
    ' 
    ' 9971 - Verifica coleção de um produto - Versão: 0
    ' 
        'On Error GoTo Err_R9971
        Dim sCurrComponent as String

        Dim C174621 As Object
        Dim C174621DataType As Short
        Dim QueryC174622 As New Object
        Dim RsC174622 As New Object
        Dim C174622DataType() As Short
        Dim RsC174622_EOF As Boolean
        Dim C174623DataType() As Short
        Dim C174632 As Boolean
        Dim C174632DataType As Short
        Dim C174637 As Object
        Dim C174637DataType As Short
        Dim C174638 As Object
        Dim C174638DataType As Short
    ...
    Comp_C174621:
        ...
        C174621DataType = 2
        C174621 = CDate(Format(GetDate(con, txn), "dd/MM/yyyy HH:mm:ss"))
        GoTo Comp_C174622

    Comp_C174622:

        ' ValidaCol
        sCurrComponent = "ValidaCol"
        QueryC174622 = con.CreateCommand()
        QueryC174622.CommandText = QueryC174622.CommandText & " " & "SELECT WF_SIGLA_PROD_AGRUP.PERTENCE_COLECAO "
        QueryC174622.CommandText = QueryC174622.CommandText & " " & "FROM AKBUSER01.WF_SIGLA_PROD_AGRUP "
        QueryC174622.CommandText = QueryC174622.CommandText & " " & "WHERE WF_SIGLA_PROD_AGRUP.Sigla_Prod = " & _ 
ConvertValue(P31846, 3, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & " "
        QueryC174622.Transaction = txn
        RsC174622 = QueryC174622.ExecuteReader()
        Dim iC174622 As Short
        ReDim C174622DataType(RsC174622.FieldCount)
        For iC174622 = 0 to RsC174622.FieldCount - 1
            Select Case RsC174622.GetDataTypeName(iC174622).ToUpper()
                Case "NUMBER", "INT32", "DECIMAL", "INT16", "DOUBLE"
                    C174622DataType(iC174622 + 1) = AKBTypeConst.cAKBTypeNumeric
                Case "DATE", "TIMESTAMP"
                    C174622DataType(iC174622 + 1) = AKBTypeConst.cAKBTypeDate
                Case "CHAR", "VARCHAR2", "LONG"
                    C174622DataType(iC174622 + 1) = AKBTypeConst.cAKBTypeString
                End Select
            Next iC174622
        RsC174622_EOF = Not RsC174622.Read()
    ...
    Comp_C174623:
        ...
        Dim tb_C174623 As DataTable = New DataTable()
        tb_C174623.Columns.Add("vRetorno" & "")
        Dim row_C174623 As DataRow = tb_C174623.NewRow()
        row_C174623(0) = C174637
        tb_C174623.Rows.Add(row_C174623)
        R9971 = tb_C174623
        ReDim C174623DataType(1)
        C174623DataType(1) = C174637DataType
        ReturnDataType = C174623DataType
    ...
    Comp_C174632:
        ...
        ' ValidaCol = 0?
        sCurrComponent = "ValidaCol = 0?"
        C174632 = (fn_ConvertValueCompiled(RsC174622(0), C174622DataType(1), AKB_DecimalPoint, False) = 0)
        C174632DataType = AKBTypeConst.cAKBTypeLogical
        If C174632 Then
            GoTo Comp_C174623
        Else
            GoTo Comp_C174638
    ...
    Comp_C174638:
        ...
        ' Atr_Zero
        sCurrComponent = "Atr_Zero"
        C174638DataType = 4
        C174637 = fn_ConvertValueCompiled(0, 4, AKB_DecimalPoint)
        C174638 = True
        GoTo Comp_C174623

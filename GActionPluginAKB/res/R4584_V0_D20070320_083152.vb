...
Public Class clsRuleDynamicallyCompiled_R4584
    ...
    Public Shared Function R4584(ByRef con As OracleConnection, ByRef txn As OracleTransaction, ByVal P12775 As Double, ByVal P12776 As Double, ByVal P12777 As Double) As DataTable
    ' 
    ' 4584 - Retorna Fator de Conversão do Item do Pedido - Versão: 0
    ' 
        'On Error GoTo Err_R4584
        Dim sCurrComponent as String

        Dim C55180DataType() As Short
        Dim C55187 As DataTable
        Dim C55187CurrentRow As Integer
        Dim C55187DataType() As Short
        Dim QueryC55188 As New Object
        Dim RsC55188 As New Object
        Dim C55188DataType() As Short
        Dim RsC55188_EOF As Boolean
        Dim C55189 As Object
        Dim C55189DataType As Short
        Dim C55190 As Object
        Dim C55190DataType As Short
        Dim C55191 As Object
        Dim C55191DataType As Short
    ...
    Comp_C55187:
        ...
        C55187 = clsRuleDynamicallyCompiled_R4400.R4400(con, txn, IIf(Not IsDBNull(C55189), C55189, System.DBNull.Value), IIf(Not IsDBNull(C55190), C55190, System.DBNull.Value), IIf(Not IsDBNull(C55191), C55191, System.DBNull.Value), fn_ConvertValueCompiled( "1", 1, AKB_DecimalPoint))
        C55187CurrentRow = 1
        iColumns = 0
        If Not IsNothing(C55187) Then
          iColumns = C55187.Columns.Count
        End If
        ReDim C55187DataType(iColumns)
        For iCol = 1 To iColumns
            C55187DataType(iCol) = clsRuleDynamicallyCompiled_R4400.GetReturnDatatype(iCol)
        Next iCol
    ...
    Comp_C55188:
        ...
        ' SelProduto
        sCurrComponent = "SelProduto"
        QueryC55188 = con.CreateCommand()
        QueryC55188.CommandText = QueryC55188.CommandText & " " & "SELECT "
        QueryC55188.CommandText = QueryC55188.CommandText & " " & "WF_PEDIDO_ITENS.Sigla_Prod , WF_PEDIDO_ITENS.Acesso , WF_PEDIDO_ITENS.Cod_Embalagem "
        QueryC55188.CommandText = QueryC55188.CommandText & " " & ""
        QueryC55188.CommandText = QueryC55188.CommandText & " " & "FROM   AKBUSER01.WF_PEDIDO_ITENS "
        QueryC55188.CommandText = QueryC55188.CommandText & " " & ""
        QueryC55188.CommandText = QueryC55188.CommandText & " " & "WHERE WF_PEDIDO_ITENS.Tp_Produto =  " & _ 
ConvertValue(P12777, 1, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & " "
        QueryC55188.CommandText = QueryC55188.CommandText & " " & "AND  WF_PEDIDO_ITENS.Pedido =  " & _ 
ConvertValue(P12775, 1, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & " "
        QueryC55188.CommandText = QueryC55188.CommandText & " " & "AND  WF_PEDIDO_ITENS.Seq_Item = " & _ 
ConvertValue(P12776, 1, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & "  "
        QueryC55188.CommandText = QueryC55188.CommandText & " " & ""
        QueryC55188.Transaction = txn
        RsC55188 = QueryC55188.ExecuteReader()
        Dim iC55188 As Short
        ReDim C55188DataType(RsC55188.FieldCount)
        For iC55188 = 0 to RsC55188.FieldCount - 1
            Select Case RsC55188.GetDataTypeName(iC55188).ToUpper()
                Case "NUMBER", "INT32", "DECIMAL", "INT16", "DOUBLE"
                    C55188DataType(iC55188 + 1) = AKBTypeConst.cAKBTypeNumeric
                Case "DATE", "TIMESTAMP"
                    C55188DataType(iC55188 + 1) = AKBTypeConst.cAKBTypeDate
                Case "CHAR", "VARCHAR2", "LONG"
                    C55188DataType(iC55188 + 1) = AKBTypeConst.cAKBTypeString
                End Select
            Next iC55188
        RsC55188_EOF = Not RsC55188.Read()
    ...
    Comp_C55189:
        ...
        C55189DataType = 0
        C55189 = RsC55188(0)
        C55189DataType = C55188Datatype(1)
        If C55189DataType = AKBTypeConst.cAKBTypeString Then
          C55189 = IIF(IsDBNull(C55189), C55189, Strings.RTrim(C55189))
        End If 
    ...
    Comp_C55190:
        ...
        C55190DataType = 0
        C55190DataType = C55188Datatype(2)
        If C55190DataType = AKBTypeConst.cAKBTypeString And Not IsDBNull(RsC55188(1)) Then
          C55190 = Strings.RTrim(RsC55188(1))
        Else 
          C55190 = RsC55188(1)
        End If 
    ...
    Comp_C55191:
        ...
        C55191DataType = 0
        C55191DataType = C55188Datatype(3)
        If C55191DataType = AKBTypeConst.cAKBTypeString And Not IsDBNull(RsC55188(2)) Then
          C55191 = Strings.RTrim(RsC55188(2))
        Else 
          C55191 = RsC55188(2)
        End If 

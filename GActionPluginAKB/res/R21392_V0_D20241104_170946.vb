...
Public Class clsRuleDynamicallyCompiled_R21392
    ...
    ' 
        ...
        Dim C579628 As Object
        Dim C579628DataType As Short
        Dim C579629 As Boolean
        Dim C579629DataType As Short
...
P82027(P82027Current) & " do pré-pedido." & Chr(13) & Chr(10) & "Ok" & Chr(13) & Chr(10)
...
ConvertValue(C508098, C508098DataType, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & " OR Data_Validade_Final IS NULL)"
    ...
    Comp_C579628:
        ...
        ' FIM_SelTaxasRS
        sCurrComponent = "FIM_SelTaxasRS"
        C579628DataType = 4
        C579628 = RsC508097_EOF
        GoTo Comp_C579629

    Comp_C579629:

        ' FIM_SelTaxasRS = 1
        sCurrComponent = "FIM_SelTaxasRS = 1"
        C579629 = (fn_ConvertValueCompiled(C579628, C579628DataType, AKB_DecimalPoint, False) = 1)
        C579629DataType = AKBTypeConst.cAKBTypeLogical
        If C579629 Then
            GoTo Comp_C508096
        Else
            GoTo Comp_C508099

...
Public Class clsRuleDynamicallyCompiled_R4852
    ...
    Public Shared Function R4852(ByRef con As OracleConnection, ByRef txn As OracleTransaction, ByVal P13685 As Object, ByVal P13686 As Object) As DataTable
    ' 
    ' 4852 - Cadastros inativos no Pedido - Versão: 0
    ' 
        'On Error GoTo Err_R4852
        Dim sCurrComponent as String

        Dim QueryC60563 As New Object
        Dim RsC60563 As New Object
        Dim C60563DataType() As Short
        Dim RsC60563_EOF As Boolean
        Dim C60564 As Boolean
        Dim C60564DataType As Short
        Dim C60565 As Short
        Dim C60565DataType As Short
        Dim C60565ReturnDataType() As Short

        Dim QueryC60566 As New Object
        Dim RsC60566 As New Object
        Dim C60566DataType() As Short
        Dim RsC60566_EOF As Boolean
        Dim C60567 As Boolean
        Dim C60567DataType As Short
        Dim C60568 As Short
        Dim C60568DataType As Short
        Dim C60568ReturnDataType() As Short

        Dim C60569DataType() As Short
        Dim C60570 As Object
        Dim C60570DataType As Short
        Dim C150357 As Object
        Dim C150357DataType As Short
    ...
    Comp_C60563:
        ...
        QueryC60563 = con.CreateCommand()
        QueryC60563.CommandText = QueryC60563.CommandText & " " & "SELECT count(*)"
        QueryC60563.CommandText = QueryC60563.CommandText & " " & ""
        QueryC60563.CommandText = QueryC60563.CommandText & " " & "FROM  AKBUSER01.WF_CLIENTES , AKBUSER01.WF_PESSOAS "
        QueryC60563.CommandText = QueryC60563.CommandText & " " & ""
        QueryC60563.CommandText = QueryC60563.CommandText & " " & "WHERE WF_CLIENTES.Cod_Cliente = " & _ 
ConvertValue(P13685, 1, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & " "
        QueryC60563.CommandText = QueryC60563.CommandText & " " & "and WF_CLIENTES.Cod_Cliente = WF_PESSOAS.Cod_Pessoa "
        QueryC60563.CommandText = QueryC60563.CommandText & " " & "and ( nvl(WF_CLIENTES.Inativo,0) = 1 or NVL(WF_PESSOAS.Ativo,0) = 0 ) "
        QueryC60563.Transaction = txn
        RsC60563 = QueryC60563.ExecuteReader()
        Dim iC60563 As Short
        ReDim C60563DataType(RsC60563.FieldCount)
        For iC60563 = 0 to RsC60563.FieldCount - 1
            Select Case RsC60563.GetDataTypeName(iC60563).ToUpper()
                Case "NUMBER", "INT32", "DECIMAL", "INT16", "DOUBLE"
                    C60563DataType(iC60563 + 1) = AKBTypeConst.cAKBTypeNumeric
                Case "DATE", "TIMESTAMP"
                    C60563DataType(iC60563 + 1) = AKBTypeConst.cAKBTypeDate
                Case "CHAR", "VARCHAR2", "LONG"
                    C60563DataType(iC60563 + 1) = AKBTypeConst.cAKBTypeString
                End Select
            Next iC60563
        RsC60563_EOF = Not RsC60563.Read()
    ...
    Comp_C60564:
        ...
        ' Cliente>0
        sCurrComponent = "Cliente>0"
        C60564 = (fn_ConvertValueCompiled(RsC60563(0), C60563DataType(1), AKB_DecimalPoint, False) > 0)
        C60564DataType = AKBTypeConst.cAKBTypeLogical
        If C60564 Then
            GoTo Comp_C60565
        Else
            GoTo Comp_C60566
    ...
    Comp_C60565:
        ...
        ' msgcli
        sCurrComponent = "msgcli"
        C60565 = 1
        C60565DataType = AKBTypeConst.cAKBTypeNumeric
    ...
    Comp_C60566:
        ...
        QueryC60566 = con.CreateCommand()
        QueryC60566.CommandText = QueryC60566.CommandText & " " & "SELECT count(*)"
        QueryC60566.CommandText = QueryC60566.CommandText & " " & ""
        QueryC60566.CommandText = QueryC60566.CommandText & " " & "FROM AKBUSER01.WF_REPRESENTANTE , AKBUSER01.WF_PESSOAS "
        QueryC60566.CommandText = QueryC60566.CommandText & " " & ""
        QueryC60566.CommandText = QueryC60566.CommandText & " " & "WHERE WF_REPRESENTANTE.Cod_Repres = " & _ 
ConvertValue(P13686, 1, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & " "
        QueryC60566.CommandText = QueryC60566.CommandText & " " & "and WF_REPRESENTANTE.Cod_Repres = WF_PESSOAS.Cod_Pessoa "
        QueryC60566.CommandText = QueryC60566.CommandText & " " & "and ( NVL(WF_REPRESENTANTE.Bloqueado,0 ) = 1 or NVL(WF_PESSOAS.Ativo,0) = 0 )"
        QueryC60566.Transaction = txn
        RsC60566 = QueryC60566.ExecuteReader()
        Dim iC60566 As Short
        ReDim C60566DataType(RsC60566.FieldCount)
        For iC60566 = 0 to RsC60566.FieldCount - 1
            Select Case RsC60566.GetDataTypeName(iC60566).ToUpper()
                Case "NUMBER", "INT32", "DECIMAL", "INT16", "DOUBLE"
                    C60566DataType(iC60566 + 1) = AKBTypeConst.cAKBTypeNumeric
                Case "DATE", "TIMESTAMP"
                    C60566DataType(iC60566 + 1) = AKBTypeConst.cAKBTypeDate
                Case "CHAR", "VARCHAR2", "LONG"
                    C60566DataType(iC60566 + 1) = AKBTypeConst.cAKBTypeString
                End Select
            Next iC60566
        RsC60566_EOF = Not RsC60566.Read()
    ...
    Comp_C60567:
        ...
        ' Repres>0
        sCurrComponent = "Repres>0"
        C60567 = (fn_ConvertValueCompiled(RsC60566(0), C60566DataType(1), AKB_DecimalPoint, False) > 0)
        C60567DataType = AKBTypeConst.cAKBTypeLogical
        If C60567 Then
            GoTo Comp_C60568
        Else
            GoTo Comp_C60569
    ...
    Comp_C60568:
        ...
        ' msgrepres
        sCurrComponent = "msgrepres"
        C60568 = 1
        C60568DataType = AKBTypeConst.cAKBTypeNumeric
    ...
    Comp_C60569:
        ...
        ' fim
        sCurrComponent = "fim"
        Dim tb_C60569 As DataTable = New DataTable()
        tb_C60569.Columns.Add("VAR1" & "")
        Dim row_C60569 As DataRow = tb_C60569.NewRow()
        row_C60569(0) = C60570
        tb_C60569.Rows.Add(row_C60569)
        R4852 = tb_C60569
        ReDim C60569DataType(1)
        C60569DataType(1) = C60570DataType
        ReturnDataType = C60569DataType
    ...
    Comp_C150357:
        ...
        ' AttFalso
        sCurrComponent = "AttFalso"
        C150357DataType = 4
        C60570 = fn_ConvertValueCompiled(0, 4, AKB_DecimalPoint)
        C150357 = True
        GoTo Comp_C60569

...
Public Class clsRuleDynamicallyCompiled_R6446
    ...
    Public Shared Function R6446(ByRef con As OracleConnection, ByRef txn As OracleTransaction, ByVal P19357 As Double, ByVal P53190 As Object) As DataTable
    ' 
    ' 6446 - Verifica se existe itens no pedido - Versão: 0
    ' 
        'On Error GoTo Err_R6446
        Dim sCurrComponent as String

        Dim QueryC89661 As New Object
        Dim RsC89661 As New Object
        Dim C89661DataType() As Short
        Dim RsC89661_EOF As Boolean
        Dim C89662 As Boolean
        Dim C89662DataType As Short
        Dim C89663DataType() As Short
        Dim C89664 As Short
        Dim C89664DataType As Short
        Dim C89664ReturnDataType() As Short

        Dim C92641 As Object
        Dim C92641DataType As Short
        Dim C92642 As Object
        Dim C92642DataType As Short
        Dim C320062 As Object
        Dim C320062DataType As Short
        Dim C320063 As Boolean
        Dim C320063DataType As Short
        Dim C320064 As Object
        Dim C320064DataType As Short
        P53190 = IIf(IsDBNull(P53190), 0, P53190)
    ...
    Comp_C89661:
        ...
        QueryC89661 = con.CreateCommand()
        QueryC89661.CommandText = QueryC89661.CommandText & " " & "Select count('X')"
        QueryC89661.CommandText = QueryC89661.CommandText & " " & "from  AKBUSER01.WF_PEDIDO_ITENS "
        QueryC89661.CommandText = QueryC89661.CommandText & " " & "where WF_PEDIDO_ITENS.Pedido = " & _ 
ConvertValue(P19357, 1, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & ""
        QueryC89661.CommandText = QueryC89661.CommandText & " " & "and " & _ 
ConvertValue(C320062, C320062DataType, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & " "
        QueryC89661.CommandText = QueryC89661.CommandText & " " & ""
        QueryC89661.Transaction = txn
        RsC89661 = QueryC89661.ExecuteReader()
        Dim iC89661 As Short
        ReDim C89661DataType(RsC89661.FieldCount)
        For iC89661 = 0 to RsC89661.FieldCount - 1
            Select Case RsC89661.GetDataTypeName(iC89661).ToUpper()
                Case "NUMBER", "INT32", "DECIMAL", "INT16", "DOUBLE"
                    C89661DataType(iC89661 + 1) = AKBTypeConst.cAKBTypeNumeric
                Case "DATE", "TIMESTAMP"
                    C89661DataType(iC89661 + 1) = AKBTypeConst.cAKBTypeDate
                Case "CHAR", "VARCHAR2", "LONG"
                    C89661DataType(iC89661 + 1) = AKBTypeConst.cAKBTypeString
                End Select
            Next iC89661
        RsC89661_EOF = Not RsC89661.Read()
    ...
    Comp_C89662:
        ...
        C89662 = (fn_ConvertValueCompiled(RsC89661(0), C89661DataType(1), AKB_DecimalPoint, False) <> 0)
        C89662DataType = AKBTypeConst.cAKBTypeLogical
        If C89662 Then
            GoTo Comp_C89663
        Else
            GoTo Comp_C89664
    ...
    Comp_C89663:
        ...
        ' Fim
        sCurrComponent = "Fim"
        Dim tb_C89663 As DataTable = New DataTable()
        tb_C89663.Columns.Add("vRet" & "")
        Dim row_C89663 As DataRow = tb_C89663.NewRow()
        row_C89663(0) = C92641
        tb_C89663.Rows.Add(row_C89663)
        R6446 = tb_C89663
        ReDim C89663DataType(1)
        C89663DataType(1) = C92641DataType
        ReturnDataType = C89663DataType
    ...
    Comp_C89664:
        ...
        ' MsgErro
        sCurrComponent = "MsgErro"
        C89664 = 1
        C89664DataType = AKBTypeConst.cAKBTypeNumeric
    ...
    Comp_C92642:
        ...
        ' Att_Falso
        sCurrComponent = "Att_Falso"
        C92642DataType = 4
        C92641 = fn_ConvertValueCompiled(0, 4, AKB_DecimalPoint)
        C92642 = True
        GoTo Comp_C89663
    ...
    Comp_C320063:
        ...
        ' Tem Tipo de Produto?
        sCurrComponent = "Tem Tipo de Produto?"
        C320063 = (fn_ConvertValueCompiled(P53190, 1, AKB_DecimalPoint, False) <> 0)
        C320063DataType = AKBTypeConst.cAKBTypeLogical
        If C320063 Then
            GoTo Comp_C320064
        Else
            GoTo Comp_C89661
    ...
    Comp_C320064:
        ...
        ' atTipoProduto
        sCurrComponent = "atTipoProduto"
        C320064DataType = 4
        C320062 = fn_ConvertValueCompiled("WF_PEDIDO_ITENS.Tp_Produto = " & P53190 , 5, AKB_DecimalPoint)
        C320064 = True
        GoTo Comp_C89661

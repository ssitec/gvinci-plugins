...
Public Class clsRuleDynamicallyCompiled_R8677
    ...
    Public Shared Function R8677(ByRef con As OracleConnection, ByRef txn As OracleTransaction, ByVal P26982 As Double, ByVal P26983 As Object) As DataTable
    ' 
    ' 8677 - Verifica Data de Validade da Proforma - Versão: 0
    ' 
        On Error GoTo Err_R8677
        Dim sCurrComponent as String

        Dim C136187 As Boolean
        Dim C136187DataType As Short
        Dim C136188 As Object
        Dim C136188DataType As Short
        Dim QueryC136189 As New Object
        Dim RsC136189 As New Object
        Dim C136189DataType() As Short
        Dim RsC136189_EOF As Boolean
        Dim C136190 As Boolean
        Dim C136190DataType As Short
        Dim C136191 As Object
        Dim C136191DataType As Short
        Dim C136192 As Short
        Dim C136192DataType As Short
        Dim C136192ReturnDataType() As Short

        Dim C136193DataType() As Short
        Dim C136194 As Object
        Dim C136194DataType As Short
        Dim C138094 As Boolean
        Dim C138094DataType As Short
        Dim C138109 As Short
        Dim C138109DataType As Short
        Dim C138109ReturnDataType() As Short

        P26983 = IIf(IsDBNull(P26983), 0, P26983)
    ...
    Comp_C136187:
        ...
        C136187 = (fn_ConvertValueCompiled(P26983, 4, AKB_DecimalPoint, False) = 1)
        C136187DataType = AKBTypeConst.cAKBTypeLogical
        If C136187 Then
            GoTo Comp_C136191
        Else
            GoTo Comp_C136194
    ...
    Comp_C136189:
        ...
        ' Sel_Dt_Val
        sCurrComponent = "Sel_Dt_Val"
        QueryC136189 = con.CreateCommand()
        QueryC136189.CommandText = QueryC136189.CommandText & " " & "SELECT WF_PRE_PEDIDO.Validade "
        QueryC136189.CommandText = QueryC136189.CommandText & " " & "FROM AKBUSER01.WF_PRE_PEDIDO "
        QueryC136189.CommandText = QueryC136189.CommandText & " " & "WHERE WF_PRE_PEDIDO.Id_PrePedido = " & _ 
ConvertValue(P26982, 1, DecimalSymbol, DateIdentifier, StringIdentifier, "dd-MM-yyyy HH:mm:ss") & " "
        RsC136189 = QueryC136189.ExecuteReader()
        Dim iC136189 As Short
        ReDim C136189DataType(RsC136189.FieldCount)
        For iC136189 = 0 to RsC136189.FieldCount - 1
            Select Case RsC136189.GetDataTypeName(iC136189).ToUpper()
                Case "NUMBER", "INT32", "DECIMAL", "INT16", "DOUBLE"
                    C136189DataType(iC136189 + 1) = AKBTypeConst.cAKBTypeNumeric
                Case "DATE", "TIMESTAMP"
                    C136189DataType(iC136189 + 1) = AKBTypeConst.cAKBTypeDate
                Case "CHAR", "VARCHAR2", "LONG"
                    C136189DataType(iC136189 + 1) = AKBTypeConst.cAKBTypeString
                End Select
            Next iC136189
        RsC136189_EOF = Not RsC136189.Read()
    ...
    Comp_C136190:
        ...
        C136190 = (fn_ConvertValueCompiled(C136191, C136191DataType, AKB_DecimalPoint, False) > fn_ConvertValueCompiled(RsC136189(0), C136189DataType(1), AKB_DecimalPoint, False))
        C136190DataType = AKBTypeConst.cAKBTypeLogical
        If C136190 Then
            GoTo Comp_C136192
        Else
            GoTo Comp_C136194
    ...
    Comp_C136191:
        ...
        C136191DataType = 2
        C136191 = CDate(Format(GetDate(con), "dd/MM/yyyy"))
        GoTo Comp_C136189

    Comp_C136192:

        ' Val_ValidadeProforma
        sCurrComponent = "Val_ValidadeProforma"
        C136192 = 7
        C136192DataType = AKBTypeConst.cAKBTypeNumeric
    ...
    Comp_C136193:
        ...
        ' RET1
        sCurrComponent = "RET1"
        Dim tb_C136193 As DataTable = New DataTable()
        tb_C136193.Columns.Add("vRet" & "")
        Dim row_C136193 As DataRow = tb_C136193.NewRow()
        row_C136193(0) = C136188
        tb_C136193.Rows.Add(row_C136193)
        R8677 = tb_C136193
        ReDim C136193DataType(1)
        C136193DataType(1) = C136188DataType
        ReturnDataType = C136193DataType
    ...
    Comp_C136194:
        ...
        ' Att_True
        sCurrComponent = "Att_True"
        C136194DataType = 4
        C136188 = fn_ConvertValueCompiled(1, 4, AKB_DecimalPoint)
        C136194 = True
        GoTo Comp_C136193
    ...
    Comp_C138094:
        ...
        ' Valida?
        sCurrComponent = "Valida?"
        C138094 = (fn_ConvertValueCompiled(C136192, C136192DataType, AKB_DecimalPoint, False)  = 6)
        C138094DataType = AKBTypeConst.cAKBTypeLogical
        If C138094 Then
            GoTo Comp_C136194
        Else
            GoTo Comp_C138109
    ...
    Comp_C138109:
        ...
        ' Msg_Err
        sCurrComponent = "Msg_Err"
        C138109 = 1
        C138109DataType = AKBTypeConst.cAKBTypeNumeric
